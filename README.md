...
### 消费过程
1. 基于**Push模型**，读线程将读到每条消息，分发到所有订阅了该消息所属主题的消费者。
2. 每个消费者对应一个阻塞队列，存放它所订阅的所有主题的消息
3. 每个生产者文件对应一个MessageReader类(Runnable),存放了每个消费者的订阅信息。每读到一条消息，取出消息的TOPIC字段，将消息分发到所有订阅该TOPIC的消费者的队列里。消费线程从队列另一端取出消息.
4. 类似于观察者模式.
