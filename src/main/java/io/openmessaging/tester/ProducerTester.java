    ...
    static class ProducerTask extends Thread {
        ...
        public void run() {
            while (true) {
                try {
                    String queueOrTopic;
                    if (sendNum % 2 == 0) { // 原来是 sendNum % 10 == 0
                        queueOrTopic = "QUEUE_" + random.nextInt(10);    // 原来是10
                    } else {
                    ...
                    // test
                    message.putProperties("host", "middleware");
                    message.putProperties("PI", 3.1415);
                    producer.send(message);
                    sendNum++;
                    if (sendNum >= Constants.PRO_MAX) {
                        DefaultProducer dp = (DefaultProducer) producer;    // 测试待删除
                        dp.flush();
                        break;
                    }
                } catch (Exception e) {
